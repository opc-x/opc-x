...
> 静态领域知识，跨圈不变。当前圈的运行上下文在战术板 GitHub Issue 里。
...
**定位：螺旋记忆。** 维护整条螺旋的运行信息，每圈结束由 O10 写入成果，下圈开始读取，实现真正的螺旋上升。

不存在则创建（已存在跳过）：
```
# {PROJECT} — 螺旋记忆

> 整条螺旋的运行信息。每圈结束 O10 写入成果，下圈开始读取。
> 当前圈的详细上下文在战术板 GitHub Issue 里。

## Vision（靶心）
[终极目标]

## 当前位置：第 1 圈
战术板：[GitHub Issue URL]
