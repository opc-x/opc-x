...
> 战术板是指挥官唯一的输入输出界面。命令写进去，结果写回来，中间的协调全部由此协议自动完成。
...
指挥官在战术板写命令（状态=待执行）
  ↓
Claude 激活 → 读战术板 → 识别待执行命令 → 路由到对应部门
  ↓
部门执行完成 → 自动触发级联 → 下一棒部门接手
  ↓
最终结果写回战术板（状态=完成 + 结果填入）
```

**指挥官不需要说"接下来做什么"——级联协议说了算。**
...
O03 PRD 完成
  └──→ O04 工程执行（自动创建 child-issue[O04-engineering]）

O04 执行完成
  └──→ O08 数据验收

O08 验收通过
  └──→ O10 复盘 → 结果写回战术板「战略结果」区

O08 验收失败
  └──→ 重新路由 O04（issue 追加失败原因）
```

### 特殊级联

```
O02 调研完成   → 结论回流 O01（支撑战略决策）
O09 财务完成   → 结论回流 O01（定价/融资决策前必查）
O06 营销就绪   → 等待 O04 产品可用（依赖，不提前执行）
```

### 执行原则

- **级联是自动的**：Claude 读到一个部门完成，立即识别下一棒并执行，不等指挥官再开口
- **唯一中断点**：需要指挥官战略判断时挂起，在战术板等待新命令
- **结果永远回写战术板**，不消失在对话里
